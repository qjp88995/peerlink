...
- `apps/signaling`（`@peerlink/signaling`）— 轻量 `ws` + zod + pino 信令服务，**全内存无数据库**。公网加固：`maxPayload` 上限 + 可选 Origin 白名单（`ALLOWED_ORIGINS`）+ ping/pong 心跳回收僵尸连接 + 单连接建房令牌桶限流（`token-bucket.ts`，超额回 `RATE_LIMITED`），均 env 可配（见 `.env.example`）。房间「待接通」即记时，无人加入的空挂房间也会在 `roomTtlMs` 后被 `reap` 回收（防长连接刷 create-room 撑爆房间表）。房间口令为 `数字-词-词`（如 `8423-河马-火山`）。
