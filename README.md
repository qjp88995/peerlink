...
基于 **WebRTC** 的点对点（P2P）文件传输系统。文件在两台设备的浏览器间**直传，不经过服务器**——服务器只做撮合（交换 SDP/ICE、局域网分组），看不到文件内容。
...
**核心设计原则**：信令与传输彻底分离，文件数据永不经过信令服务；`packages/protocol` 是协议的唯一事实源（zod schema 定义所有信令消息与分片帧，前后端共享同一套类型与运行时校验）。
...
| `peer-connection`  | `core/peer-connection.ts`             | 封装 `RTCPeerConnection`、ICE、DataChannel 建立                        |
| `transfer`         | `core/sender.ts` / `core/receiver.ts` | 文件分片、背压控制、组装、CRC32 校验                                   |
| `storage`          | `core/storage/*`                      | 接收端写入抽象（File System Access / ZIP / 内存 Blob，按能力探测选择） |
| `state`            | `state/store.ts`                      | zustand 连接/传输状态                                                  |
| `ui`               | `features/*` / `routes/*`             | React 组件 + TanStack 文件式路由                                       |

后续 App 可替换 `storage` / `ui`，复用 `protocol` / `transfer` / `peer-connection` / `signaling-client`。
...
浏览器打开 http://localhost:5173 ，在发送端生成 `/r/<code>` 链接，接收端打开即可配对。
...
接收端写入按浏览器能力降级：File System Access（Chromium，选目录边收边写）→ ZIP 流式打包（Safari/Firefox 多文件/文件夹）→ 内存 Blob（小单文件兜底）。
...
- **前端**：React 19 · Vite · Tailwind CSS v4 · TanStack Router · zustand · sonner · lucide-react · zip.js · qrcode
